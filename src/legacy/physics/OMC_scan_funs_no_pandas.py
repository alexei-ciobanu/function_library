...
def digitize_output(dc,N_bins=40000,verbose=True):
    
    lbnd = np.min(dc['pzt'])
    ubnd = np.max(dc['pzt'])*(1+np.spacing(1.0))
    data = dc['pzt']
...
#         print(len(np.unique(bin_ind)),nbins)
    ...
    # groupby digitized_pzt
    pzt_groups = {}
    pzt_bins = digitized_pzt
    omc_vals = dc['omc']
    for pzt_bin, omc_val in zip(pzt_bins, omc_vals):
        if pzt_bin not in pzt_groups:
            # new bin: create group
            pzt_groups[pzt_bin] = []
        # add omc_val to the pzt_bin
        pzt_groups[pzt_bin].append(omc_val)
    
    # take mean across omc_vals in the same pzt_bins
    group_mean = []
    for pzt_bin in pzt_groups:
        group_mean.append([pzt_bin,np.mean(pzt_groups[pzt_bin])])
    # print(result)

    # sort array by pzt val (so that it prints nicely)
    dn = np.array(group_mean)
    sort_ind = np.argsort(dn[:,0])
    dc2 = {}
    dc2['pzt'] = dn[sort_ind,0]
...
,remove_negative=True,digitize_pzt=True,N_bins=40000,q=6,keep_range=[0,1],dir=1
,gradient_tolerance=0.01,plot_output=True,linear_yscale=True,verbose=False,export_csv=True,export_plot=True):
    ...
    if digitize_pzt:
        dc3 = digitize_output(dc2,verbose=verbose,N_bins=N_bins)
    else:
        dc3 = dc2.copy()
    ...
    if linear_yscale:
        plt.plot(dc3['pzt'],dc3['omc'],ms=1,lw=1)
    else:
        plt.semilogy(dc3['pzt'],dc3['omc'],ms=1,lw=1)
    ...
    if export_csv:
        with open(out_filename+'.csv', 'w') as csvfile:
            ...
            wr.writerow(list(dc3.keys()))
            for row in zip(dc3['pzt'],dc3['omc']):
                wr.writerow(row)
...
def iter_bootstrap(dc1,old_fit_dc,f0,RF=0,show=False,delta=None):    
    ydata = dc1['omc']
    if delta is None:
        delta = min(ydata)*3
    
    peaks_loc = peakdet(ydata,delta=delta,show=show)
    locs_dc = {}
    for key in dc1:
...
def stage2(dc,deltaf=5.813185e+07,FSR=2.649748e+08,delta_upper=None,delta_lower=None,verbose=False,RFs=None,show=False,plot=False):
    '''
    ...
    if RFs is not None:
        for RF in RFs:
            for i in np.arange(0,n_zeros,1):
                fit0 = iter_bootstrap(f0,fit0,FSR*i,RF=RF,delta=delta_lower,show=show)
    ...
    for i in np.arange(0,n_zeros,1):
        ...
        fit0 = iter_bootstrap(f0,fit0,hom_freq_1,delta=delta_lower,show=show)
        # update frequency model from first order peak before trying to find second
        f0,p = update_freq_model(f0,fit0,poly_order=poly_order,plot=plot)
        fit0 = iter_bootstrap(f0,fit0,hom_freq_2,delta=delta_lower,show=show)
...
def find_peak_ind(ydata,delta=None,verbose=True):
    # could use a different backend here
    if delta is None:
        delta = np.min(ydata)*2
    
    if verbose:
        show = True
    else:
        show = False
    peak_ind = peak_finder.peakdet(ydata,delta=delta,show=show)
    return peak_ind

def get_peak_labels(dc,freq_data,height_data,delta=None,verbose=True):
    peak_ind = find_peak_ind(height_data,verbose=verbose)
...
def get_even_peak_heights(dc_cs,max_confidence=1e-3):
    ...
    for key in dc_even:
        dc_even[key] = dc_cs[key][confidence_mask]
...
def stage3(xdata,ydata,FSR=2.649748e+08,deltaf=-5.813185e+07,RFs=[],maxtem=8,delta=None,confidence_cutoff=1e-3\
